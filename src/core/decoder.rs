...
impl Decoder {
    ...
    pub fn decode(raw_instruction: RawInstruction) -> Instruction {
        ...
        match opcode {
            ...
            0x3 | 0x4 | 0x5 | 0x6 | 0x7 | 0xE => {
                decode_reg_const_op(raw_instruction)
            },
            0x8 | 0xA => { // LDD / STD
            ...
            0xF => {
                decode_branch_skip_status_op(raw_instruction)
            },
            _ => {
                unreachable!()
            }
...
fn decode_misc_mult_op(raw_instruction: RawInstruction) -> Instruction {
    ...
    match sub_op {
        0x0100 => {
            Instruction::TwoRegOp{op: alu::MOVW_OP, rd: rd * 2, rr: rr * 2}
        },
        0x0200 => {
            Instruction::TwoRegOp{op: alu::MULS_OP, rd: rd + 16, rr: rr + 16}
        },
        0x0300 => {
            match (rd < 8, rr < 8) {
                (true, true)  => Instruction::TwoRegOp{op: alu::MULSU_OP, rd: rd + 16, rr: rr + 16},
                (true, false) => Instruction::TwoRegOp{op: alu::FMUL_OP, rd: rd + 16, rr: rr + 8},
                (false, true) => Instruction::TwoRegOp{op: alu::FMULS_OP, rd: rd + 8, rr: rr + 16},
                (false, false) => Instruction::TwoRegOp{op: alu::FMULSU_OP, rd: rd + 8, rr: rr + 8}
            }
        }
        _ => unreachable!()
    }
...
fn decode_branch_skip_status_op(raw_instruction: RawInstruction) -> Instruction {
    if raw_instruction & 0x0800 == 0 {
        ...
        let offset = if u_offset < 0x40 { u_offset as i8 }
